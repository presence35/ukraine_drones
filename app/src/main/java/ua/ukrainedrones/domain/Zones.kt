 ...
 * measured → nominal estimate). Honors per-type reach caps. Fast threats tier by ETA, slow
 * threats by plain distance — except AVIATION: a MiG-31K takeoff is a country-wide Kinzhal
 * warning, so within reach it always rings INNER regardless of ETA or zone thresholds (the
 * user's only opt-out is the type's bell). This is the single source of truth for zone
 * tiering — used by both MainViewModel and AlertService.
 */
fun zoneTier(t: Threat, distKm: Double, speedKmh: Double?, params: ZoneParams): ThreatZone? {
    if (distKm > reachKm(t.type)) return null
    if (t.type == ThreatType.AVIATION) return ThreatZone.INNER
