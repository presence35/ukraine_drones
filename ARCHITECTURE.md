...
| `IconCatalog.kt` | Single source of truth for threat icon drawables: `classicRes` (vector set), `res(type, set)` (vector, photo, or army set — photos live in `iconpacks/photo/drawable-nodpi/threat_photo_*.png`, army in `iconpacks/army/drawable-nodpi/threat_army_*.png`), per-set baked-in facing `photoBaseDeg`/`armyBaseDeg` (also via `baseDeg(type, set)` for map rotation), and the `ThreatIcon` composable that letterboxes raster sets in square slots. Replaces the old per-file `iconFor`/`iconResFor`/`threatIconRes` mappings. Icon-pack assets live under `app/src/main/iconpacks/` (classic + photo + army) and are merged into the res namespace via `res.srcDirs` in `build.gradle.kts`. |
