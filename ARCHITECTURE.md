...
| File | Responsibility |
| --- | --- |
| `NeptunClient.kt` | `object` singleton. NEPTUN WebSocket (fast-first backoff 1–3s → capped 15s via `reconnectDelayMs`, keep-alive/watchdog) + REST merge → `StateFlow<NeptunState>`; `removedThreats` SharedFlow; `retryNow()`; hosts `OFFLINE_GRACE_MS` (5 s — sub-grace drops are invisible in the connection log; the UI pill and notifications reflect drops immediately — see mirror rule). A `ConnectivityManager.NetworkCallback` watches the OS network: on a validated link reconnects run fast (1–15s); on a provably-dead link the reconnect backoff widens to `NO_NETWORK_RECONNECT_MS` (60 s — retrying against a dead link is pure radio burn) and the callback's `onCapabilitiesChanged` kicks an immediate retry. `NeptunState.degraded` (orange middle state) is true while connected but the stream has been quiet for `DEGRADED_STALE_MS` (30 s, below the 15s REST-refresh / 45s watchdog) — the "threats are delaying" warning. `markUserShot(id)` records map long-press fake kills in `NeptunState.userShotAt`; the snapshot handler keeps those ids alive in memory through the grace window so a quick same-id respawn redraws in place instead of re-entering as a new threat. Socket generation guard (`AtomicInteger`) prevents stale callbacks from superseded sockets. `reconnectStartMillis` is persisted across process kills so 3/6/10/20-min milestones survive restarts. Pure `reconnectDelayMs` (tested). |
...
| `WidgetSnapshot.kt` | `WidgetSnapshot` + pure `computeWidgetSnapshot(...)` — deterministic projection of threat state for the widget, from shared domain functions only (`ThreatEvaluator.evaluate`, `distanceMeters`, `focusAttribution`, `inOblast`). Counts + per-type `typeCounts` mirror the footer-strip semantics; `primaryThreat` = nearest live threat (id + position) so the widget can reveal it; `sourceOnline` is grace-filtered like the app pill and `sourceDegraded` mirrors `NeptunState.degraded`. Never re-derives zone/tier logic (mirror rule). Tested by `WidgetSnapshotTest`. |
...
| `ConnectionStatus.kt` | Connection pill (online / degraded-orange / offline) + system-status dialog: per-source dot, TEMP force-offline toggle, legend, attribution link, and a prominent "Logs" button (in the dialog header) opening the Logs screen. The orange middle state mirrors `NeptunState.degraded` — connected but the stream is quiet (threats delaying) — distinct from `SourceStatusRow`'s amber accent. |
...
| `widget/WidgetUpdater.kt` | `object` singleton. Started by `AlertService` (the already-running monitor, ~zero marginal battery). Guards on `hasPlacedWidgets()` before starting the pipeline. Combines `NeptunClient.state` + `LocationTracker.location` + zone/follow/pin/language/type-gate prefs + a 30s clock -> `computeWidgetSnapshot` -> persists to the `widget_snapshot` DataStore and calls `updateAll()` only when a widget is actually placed. Persists `primaryThreat` (id/lat/lon/type) so the widget can reveal the nearest threat. Exposes `readSnapshot`/`readLang`/`readIconSet` 
...
- **Official alerts come from NEPTUN only** (the alerts.com.ua backup was removed — a dead
  ...
  **Connection health is three-tier, not two.** Green online / red offline stay grace-filtered
  as before; the new orange **degraded** middle state (`NeptunState.degraded`) fires while the
...
| --- | --- | --- |
| NEPTUN offline | Offline pill + monitor notification with Retry; `retryNow()`; last-known oblast alerts held; drop persisted (`offline_pending_since`) across restarts; reconnect start persisted for milestone continuity. | `NeptunClient`, `AlertService` |
