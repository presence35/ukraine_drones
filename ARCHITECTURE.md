...
| `ThreatPopupCard.kt` | Threat detail popup in two sizes (small / large): type, region, threat-level gauge, a neutral distance/ETA/speed pill trio, precision, reliability (3-segment bar in full / "R" (UA "Д") bar on a bottom row with the skull bar in small), wave size, time since seen. The `neutralized`/`neutralizing` compact variant announces a resolved threat ("Neutralized" / "Neutralizing enemy…"). |
