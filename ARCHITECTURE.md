...
| `Strings.kt` | EN-only `StringSet` source (`en*()`); UA derived as `EN.copy(UA) + uaManualPatch // MANUALLY KEPT` (bulk-translated before release, `tools/sync_ua.py --check` guards staleness via EN snapshot/hash); `formatRelativeTime`/`formatDateTime` per app language. |
...
| `FirstLaunchWizard.kt` | The 5-page first-run wizard (language+tips → threat care → location follow-me/city-pick → zone controls → core features), extracted from MainScreen; thin yellow→blue progress bar on the Next pill's bottom edge, whole pill dims while disabled. *Note:* the zone-controls step edits the REAL slow red/yellow radii + armed toggles (wired to the same ViewModel setters as the map) and shows a static replica of the map's bottom strip (shelter pill, zone buttons, scale + attribution) — reusing `ShelterButton`/`ZoneButtons`/`ScaleIndicator` from MainScreen; every interactive control ticks via `pressTick`. |
