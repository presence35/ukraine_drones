...
- Single-module Android app (`:app`) â€” a live air-threat map for Ukraine.
...
widget/`) while keeping a **single flat package** `ua.ukrainedrones` â€” so any file can reach
...
The UI and the background service are **independent consumers of shared inputs** â€” both
re-derive state from the same singletons, never from each other.

```
                    â”Œâ”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”
 NEPTUN WS â”€â”€â”€â”€â”€â”€â”€â”€â–ºâ”‚                      â”‚
 NEPTUN REST â”€â”€â”€â”€â”€â”€â–ºâ”‚     NeptunClient     â”‚
                    â””â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”¬â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”˜
                               â”‚ StateFlow<NeptunState>
...
```
ZonePrefs â”€â”€â”€â”€â”€â”€â”€â”€â”¬â”€â”€â–º MainViewModel        Shared domain logic (call, don't duplicate):
                  â””â”€â”€â–º AlertService         Zones.kt (zoneTier) / Prediction.kt (predictPosition)
LocationTracker â”€â”€â”¬â”€â”€â–º MainViewModel        NightMode.kt / Cities.kt (focusAttribution)
                  â””â”€â”€â–º AlertService
```
...
- **Threat ingest.** NEPTUN WS + REST merge in `NeptunClient` â†’ `StateFlow<NeptunState>`;
...
- **Position prediction** (both consumers):
  `ThreatSpeedTracker.record(...)` â†’ `estimateWithSource(id, t)` (server â†’ measured â†’ nominal)
  â†’ `predictPosition(t, speed, now)` (dead-reckon **only tracks with a real velocity** â€”
  `bearingDeg` + `speedKmh`, matching NEPTUN's `flying`; everything else holds the raw fix â€”
  capped at the per-type horizon/ghost) â†’ distance to focus â†’ `zoneTier` (slow:
...
- **Update flow.** `UpdateManager.check()` â†’ `Available` â†’ `download()` (progress) â†’
  `buildInstallIntent()` â†’ system installer. `AlertService` also checks silently every day at
...
| File | Responsibility |
| --- | --- |
| `MainActivity.kt` | Single activity; dark theme; starts `AlertService`; legacy osmdroid cache cleanup. *Note:* locationâ†’notification permissions defer until first-run onboarding resolves; "Later" sets `permission_prompt_deferred` (re-armed each cold start). |

### Data ingress (NEPTUN)

| File | Responsibility |
| --- | --- |
| `NeptunClient.kt` | `object` singleton. NEPTUN WebSocket (fast-first backoff 1â€“3s â†’ capped 15s via `reconnectDelayMs`, keep-alive/watchdog) + REST merge â†’ `StateFlow<NeptunState>`; `removedThreats` SharedFlow; `retryNow()`; hosts `OFFLINE_GRACE_MS` (5 s â€” sub-grace drops are ignored by the pill and the connection log; deliberate) and `USER_SHOT_GRACE_MS` (3 s). `markUserShot(id)` records map long-press fake kills in `NeptunState.userShotAt`; the snapshot handler keeps those ids alive in memory through the grace window so a quick same-id respawn redraws in place instead of re-entering as a new threat. 5s watchdog â†’ `ConnectionLog`. Pure `reconnectDelayMs` (tested). |
...
| File | Responsibility |
| --- | --- |
| `MainViewModel.kt` | `AndroidViewModel`. Combines NEPTUN + GPS + prefs + 1s clock (flourish data/policy live in `flourish/Flourish.kt` â€” `FlourishRecord`/`FlourishShow` and the `FlourishPolicy` gates used by `buildUiState`) â†’ `StateFlow<UiState>`; drives the update flow (daily start check, a Settings-open check that raises `updateReminderTick` â€” a snackbar with a Download action â€” instead of a clickable toast, which Android can't make touchable, plus manual check/download/install); UI-side copy of zone/focus/alert logic (tradeoffs); `neutralizeThreat` long-press hook. The neutralizing-card swap fires only while the map screen is visible (`mapVisible`, set from the visible screen) and the shelter overlay is not up (`shelterModeActive`, set via `setShelterModeActive`) â€” nothing should steal focus from the shelters; it does play during an alert because the threat was already on screen. `neptunDown` ignores sub-grace drops. *Note:* the map's `focusLocation` falls back to Odesa before the first GPS fix so the first visual is complete â€” this is **UI-only**; `focusAttribution`/alerts still use the real fix or pin. A `flourish` request (tally-tap replay, from `AlertService`) flows to `MapView`. |
...
| `DebugLog.kt` | `object` singleton. Persisted audit trail (last 500 decisions, rolling 24h window) written by `AlertService`, read by the Logs screen. Records every alert/threat decision in the active region â€” official on/off, zone entries, region threats â€” with day/night and effective sound, whether a notification was shown and why not. `DebugLog.sweep` runs once per service tick and is **read-only for the decision path**: it describes the service's own computed maps (`zoneThreats`/`alertable`/`knownZones`/`postedId`), never re-derives formulas. Pure `computeSweep`/serialize/parse (tested). *Note:* the whole feature is additive â€” removing it is deleting the write hooks + this object + `DebugLogScreen`. |
| `Prediction.kt` | `LatLng`, `distanceMeters`, per-type `staleAfterMs`/`isExpired`, `predictPosition` dead-reckoning (only `flying` tracks with real velocity glide), `motionHeading` (server bearing, then heading, then measured fix-track â€” shared by motion + facing), `ThreatSpeedTracker` (shared thread-safe singleton so every consumer measures the same speed/track). |
...
| File | Responsibility |
| --- | --- |
| `Zones.kt` | `ThreatZone`, `ZoneParams`, `FastThreatTypes`, `zoneTier(...)` â€” the single source of truth for tiering â€” plus `etaMinutes`, `reachKm`. AVIATION bypasses ETA tiering (always INNER within reach). |
...
| `ThreatLevel.kt` | `ThreatLevelModel` â€” experimental 0â€“10 gauge (severity Ã— distance Ã— reliability Ã— sources Ã— count Ã— quality Ã— staleness Ã— ETA). |
| `Cities.kt` | ~483 places grouped by oblast in three zoom tiers (`CityTier`: 26 curated MAJOR always / MEDIUM from mid-zoom â€” 14 curated non-seats plus any place with `pop` â‰¥ 50k auto-promoted in `ALL` / rest MINOR up close; non-curated places derived from GeoNames CC BY 4.0 via `tools/gen_cities.ps1`, 2 km dedupe, same-name towns resolved by population; a few district rows carry no `pop` so inherited parent-city figures never promote them) + `CityLabelOverlay` (colors labels red for the `redCities` set â€” scope-aware: whole oblast by default, city-level when the City scope is on); EN names from the app's own ÐšÐœÐ£ â„–55 transliteration; `focusAttribution` maps focus point â†’ oblast stem via `cityOblast` (majors only). |
| `Transliteration.kt` | Official ÐšÐœÐ£ â„–55 Ukrainianâ†’Latin romanization (the EN gate). |
| `ZonePrefs.kt` | `AppLanguage`/`ThreatCardSize`/`ThreatIconSet` + DataStore store (`zone_prefs`): all toggles/thresholds/language/follow/pin/visibility, night config, and â€” problematically â€” serialized `ConnectionLog`, offline-restore state, onboarding flags. Also `threatMapFlow`/`threatAlertFlow`; the resolved-threat tally's focus-oblast default + "All of Ukraine" opt-in (`neutralized_tally_all_ukraine`), and the daily-update notify marker (`last_notified_update_code`). *Note:* god object mixing prefs with persisted state â€” split candidate (tradeoffs). |
...
| `app/version.properties` | `versionCode`/`versionName` â€” source of truth for the build + `version.json`. |
...
- **Official alert announces once per episode, surviving service restarts.** `AlertService`
  ...
  showing (`alertNotificationShowing()`) â€” a tapped/swiped alert is never resurrected as a new
...
- **Widgets read snapshots, never evaluate.** The home-screen widget is a passive renderer of
  `WidgetSnapshot`, computed solely by `WidgetUpdater` via `computeWidgetSnapshot` â€” which calls
...
- **Threat type gating.** `threatMapFlow` gates map rendering, `threatAlertFlow` gates alerts â€”
  ...
  footer strip; a type with alerts off stays fully mapped (never dimmed â€” dimming is
  staleness-only) but is omitted from the footer strip, with a red crossed bell on its popup.

- **Focus point.** `followMe` â†’ camera + zones + alerts centre on GPS, else the pinned city;
  pinning disables follow-me. Attribution via `focusAttribution` â†’ `cityOblast` stem match,
  **major cities only** â€” the ~300 minors are map-context, never banner/alert. *First-launch
  visual:* before the first GPS fix the **map's** `focusLocation` falls back to Odesa (complete
  first screen) â€” this is UI-only; `AlertService`/`focusAttribution` never use the fallback, so
  no fake region alert is ever produced.

- **Zone tiering.** `zoneTier(t, distKm, speedKmh, ZoneParams(slowRedKm, slowYellowKm,
  fastRedMin, fastYellowMin))`. Fast types (`FastThreatTypes`) tier by ETA (â‰¤ fastRedMin â†’
  INNER, â‰¤ fastYellowMin â†’ OUTER); slow types by distance (â‰¤ slowRedKm â†’ INNER, â‰¤ slowYellowKm
  â†’ OUTER). Slow distance is to the **confirmed raw fix**, never the dead-reckoned position, so
  the drawn circles and alerts always agree. Speed from `ThreatSpeedTracker` (server â†’ measured
  â†’ nominal); a fast threat with no usable speed never tiers. **AVIATION is the exception:** a
  MiG-31K takeoff alert is a country-wide Kinzhal warning â€” within `reachKm` it always rings
  ...
  Armed bells are per groupÃ—tier (slow/fast Ã— red/yellow), stored for day and night, resolved
  per tick by `effectiveArmed`. **Slider coupling:** the yellow threshold is relative to red â€”
  setters clamp yellow to `red+2 â€¦ max` (slow 1â€“20 km red / yellow â‰¤50; fast 1â€“5 min red /
  yellow â‰¤20) in `ZonePrefs`; the UI sliders derive the yellow range from the red value live.

- **Expiry / ghosts.** `staleAfterMs` per type (90s ballistic â€¦ 300s UAV). Stale threats stay
  ...
  without fix refreshes, so age-based expiry would kill every real alert before it rang â€” only
  ...
  for the neutralizing card (flips at `DEATH_EXPLOSION_START_MS`, fades across the explosion) â€”
...
- **Threat facing always matches its motion.** `predictPosition` (dead-reckoning) and
  ...
  only glides tracks with a real velocity (`bearingDeg` + `speedKmh`, i.e. `Threat.flying`) â€”
...
- **Place names transliterate, never translate.** Any Ukrainian proper noun in the EN UI â†’
  `Cities.uaToEn` â†’ `Transliteration.transliterate`; military vocabulary is hard-coded
...
- **Official alerts come from NEPTUN only** (the alerts.com.ua backup was removed â€” a dead
  project). `NeptunState.oblastAlerts` is NEPTUN's list, held (never cleared) while the socket
  is down: an outage must not fabricate "alert ended" / false all-clear â€” the truth arrives on
...
- **Official alert is region-latched, never focus-bound for its end.** The official-alert
  ...
  ringing. Switching the focus away (pin â†’ GPS, or to another city) to a non-alerting region
  must **not** announce an all-clear for the old region nor keep its banner â€” it silently drops
...
- **Mitigation:** shared pure functions own all decision logic â€” `zoneTier`,
  ...
  service also measure the **same** speed history â€” they can't disagree near a zone boundary.
- **Future:** a shared `ThreatEvaluator` returning evaluated threats instead of two giant
  consumers reconstructing the same logic.

### Flat package

All code in one flat package â€” deliberate for now (Package structure). Cost grows with file
count; migrate by subsystem when it outgrows maintainability.

### Foreground service instead of backend/push

No intermediate server buffers anything, so nothing is missed server-side â€” but the app must be
...
```
AppPrefs
â”œâ”€â”€ MapPrefs / ThreatPrefs / AlertPrefs / NightPrefs / UiPrefs / SystemPrefs
```
...
JUnit unit tests in `app/src/test/java/ua/ukrainedrones/`. Invariant â†’ test: the mirror paths
(`zoneTier`, `predictPosition`, staleness) are pinned by `ZonesTest` / `PredictionTest`, which
both consumers rely on.

- `PredictionTest.kt` â€” `predictPosition`, `distanceMeters`, staleness, speed tracking.
- `CitiesTest.kt` â€” city-list integrity; majors-only `nearestCity`/`focusAttribution`.
- `ThreatTest.kt` â€” JSON parsing, type mapping, course translation.
- `TransliterationTest.kt` â€” ÐšÐœÐ£ â„–55 romanization, no semantic translation, digraph rules.
- `ThreatLevelTest.kt` â€” threat-level scoring.
- `ZonesTest.kt` â€” slow km / fast min tiering, `etaMinutes`, `reachKm`, AVIATION override, null-speed fast.
- `UpdateManagerTest.kt` â€” `versionNameGreater`.
- `NeptunClientTest.kt` â€” reconnect backoff.
- `NightModeTest.kt` â€” night-window resolution + effective params/armed.
- `ConnectionLogTest.kt` â€” episode-commit rules (grace window, blips, recovery, ring-buffer cap).
- `DebugLogTest.kt` â€” serialize/parse round trip, ring-buffer cap, auto-clear, `computeSweep` verdicts (fired/coalesced/bell-muted, steady-state dedup, tier escalation, exits, stale/type-off region rows).
- `VibrationTest.kt` â€” `vibrationPattern` levels.
- `StringsFormatTest.kt` â€” `formatDateTime` per-language correctness.
- `TestThreats.kt` â€” shared `threat(...)` builder helper.

Run: `.\gradlew.bat :app:testDebugUnitTest`

## Build & release

- `.\gradlew.bat :app:assembleDebug` â€” debug APK (no secrets needed).
- `.\gradlew.bat :app:release` â€” bumps version, builds release APK, uploads APK + generated
...
- **UI samples the stream.** MainViewModel reads NeptunClient.state.sample(120) for the UI
  path only; AlertService keeps consuming the raw flow (mirror rule unaffected).
- **Clock is not state.** 
...
- **Popup clock is a leaf.** ThreatPopupCard's elapsed-time text comes from
  ThreatElapsedText, a small composable with its own 1s clock â€” the card body does not
